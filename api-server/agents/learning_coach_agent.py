...
- **重要: ask_ai_application_*ツールには`start_new_conversation`引数があります。ユーザーが「新しく案件を探したい」「別の条件でもう一度」「前回とは違う条件で探して」のように、前回までに確認済みの条件(職種・予算・納期・プラットフォーム等)を引き継がず一から検索し直したい意図を示した場合は`start_new_conversation=true`にしてください。単に前回の続きとして条件を絞り込んだり質問に答えているだけの場合はfalse(省略可)のままにしてください。これを正しく判定しないと、ユーザーが新しい条件を伝えたつもりでも古い条件のままの検索結果が返ってしまいます**
