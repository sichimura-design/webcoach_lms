 ...
 * 学習日カレンダー（月表示）。
 *
 * 🔴 月表示にしているのは、同じカードに「今月の学習日数」を出す要件があるため。
 *    直近5週表示だと月境界をまたいで、塗られたセルの数と表示している数字が一致しない。
 *
 * 🔴 色の意味づけ:
 *    primary    = 学習した日
 *    streakOff  = 機会があったのに学習しなかった日（過ぎた未学習日）
 *    transparent= まだ判定されていない日（未来）
 *    MyPage.tsx の週間ドット（studied ? primary : streakOff）の延長で、
 *    TimerDial のトラックにも streakOff を使うので画面全体で意味が一貫する。
 */
          ...
          return (
            <div
              ...
              style={{
                ...
                boxShadow: isToday
                  ? `0 0 0 2px ${color.surface}, 0 0 0 3px ${color.primaryBorder}`
                  : undefined,
              }}
            >
              {day}
