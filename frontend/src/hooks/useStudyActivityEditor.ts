 ...
 *
 * 🔴 useStudyActivities（一覧）と分けている理由:
 *    書き換えは「学習履歴の行」と「カレンダーの日別パネル」の2箇所から起きる。
 *    useStudyActivities はページング状態を抱えた一覧専用なので、日別パネルから
 *    それをもう1つ生やすと、見えていない一覧を裏で読み直すことになる。
 *
 * 🔴 楽観更新をしない理由:
 ...
 *    useStudyStats / useStudyActivities / useMonthActivities が全部これを購読しているので、
 *    KPI・カレンダーの濃淡・推移グラフ・履歴が一斉に追随する。
 *    「履歴では消えたのにストリークが減らない」を構造的に起きなくするのがこの1行。
 * ============================================================
 */
