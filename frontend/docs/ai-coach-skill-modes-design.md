...
| 状態 | URL | 中身 |
|---|---|---|
| ホーム | `/ai-coach` | 大きな入力欄 ＋ 全AIアプリ（カテゴリ別カード） |
| メインチャット | `/ai-coach?session=page:1` | 通常のAIコーチ。機能一覧（AiSkillDock）は入力欄の上に縮めて置く |
| 専門モード | `/ai-coach?session=page:2` | 機能名・1行説明・必要な入力・クイックアクション・参照情報・戻る導線 ＋ 同じ AiSkillDock |

独立した一覧ページ（旧 `?view=catalog`）は持たない。アプリは11個しかなく、一覧を別画面に分けると7個目以降が存在に気づかれないうえ、使うのに1ステップ余計にかかるだけだった。ホームでは入力欄の下に全件をカードで並べ、会話中は AiSkillDock を**その場で展開**する。
...
ホームのカードには **何ができるか / 何を入力するか / どんな場面で使うか** を必ず出す（[AiCoachHome.tsx](../src/components/aicoach/AiCoachHome.tsx) の `renderCard`）。文言はすべて [types/aiSkill.ts](../src/types/aiSkill.ts) の `AI_SKILL_META` が唯一の情報源で、提案カードの説明もここから引く。別に説明文を持つと、一覧と提案で説明が食い違う。

### 7.1 会話中の機能一覧は、その場で開く

[AiSkillDock.tsx](../src/components/aicoach/AiSkillDock.tsx) は既定で3件のチップだけを出し、「すべて見る」で**全11件をカテゴリ別にその場で展開する**。

- 🔴 **「すべて見る」で画面遷移しないこと。** 以前はホームへ戻していたが、残り8個を見るためだけに会話から抜ける必要があり、戻る手間もかかっていた。
- 展開パネルには必ず高さの上限（`min(180px, 30vh)` ＋ 内部スクロール）を置く。`AiCoachPane` は縦フレックスで会話部が `flex:1 / minHeight:0`、footerSlot が `flexShrink:0` なので、上限が無いと11件がメッセージ領域を押し潰す。
- ドックは**専門モードでも出す**。隠すと別のアプリへ移る道がホーム経由だけになる。いま使っているアプリは「・使用中」として `disabled` で示す。
...
- **一覧（AiSkillDock）から専門モードを始めた場合**は、必要な文脈（教材・課題基準・添付画像・引用）だけをコピーした子セッションを作る。引き継いだことは子の会話に system メッセージとして残す
- **会話中の提案カードを受け入れた場合**は、同じ会話のままモードが切り替わる。「いまの話の続き」なので、別セッションへ移すと文脈が切れて読みにくい
- **専門モードからさらに別のアプリを選んだ場合**は、その会話の子ではなく**兄弟**として作る（親は自分の親）。[ConversationList.tsx](../src/components/aicoach/ConversationList.tsx) は roots とその子の2階層しか描かないので、孫を作ると履歴一覧からそのまま消える
