...
- モックは `REACT_APP_ENABLE_MOCKS=true` のときだけ有効。
  - **ローカル（`npm start`）**: `frontend/.env.development` で既定 ON → 実 Cognito / 実 BFF 無しで完全オフライン起動。まず `/login` が表示され、任意のメール/パスワードで「ログイン」すると擬似ユーザーとして `/mypage` に入れる（`frontend/src/mocks/mockAuth.ts`）。
