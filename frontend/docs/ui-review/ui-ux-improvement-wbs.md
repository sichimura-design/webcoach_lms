...
| F8-04 | 保守 | 管理・コーチ画面 | デザイン統一 | 管理12画面・受講生一覧の `#E86D78`（34箇所／12ファイル）を `#E0213A` 系に置き換える | 管理・コーチ画面だけサーモン系の別ブランドになっている | （受講生には影響しない。運用担当者から見て別サービスに見える） | 受講生フローの色統一が終わった後、まとめて置き換える。`ToastContext` のエラー色と `.moodle-content` のリンク色もここで揃える | 管理・コーチ画面のスクリーンショットに `#E86D78` が残っていない | P3 | M | F0-10, F0-16, F0-17 | `frontend/src/components/admin/`, `frontend/src/components/coach/CoachStudentsPage.tsx`, `frontend/src/contexts/ToastContext.tsx`, `frontend/src/index.css` | コーチ3画面のみ対応済み(2026-09-08 / dev/coach-ui) ／ 管理12画面と ToastContext は未着手 |
